...
==== Implementation
`**List**` is  instantiated by either `ListCommandParser#parse(String args)`,
...
===== Aspect: Using Predicates to improve extendability of the feature in the future
...
** Cons: Not easily extendable as in the future if we want to filter by new arguments, more predicates will have to be written.
